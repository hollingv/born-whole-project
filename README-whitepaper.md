...
BWP encourages participating organizations to publish content at consistent URL paths, which supports future automated aggregation features planned for news, events, and legal case tracking.
...
The knowledge base is a curated collection of hand-written plain text files stored in `site/kb/`, one per featured organization. Each file contains a factual summary of the organization's work, key arguments, and relevant information about genital cutting — written in the contributors' own words rather than scraped from third-party sites. This approach ensures legal clarity around copyright, consistent quality, and stability between deployments.

The knowledge base powers the AI question-answering feature. A manifest file (`site/kb/manifest.json`) lists all KB files and is regenerated by running `bwctl harvest`. Adding a new KB file and regenerating the manifest is all that is required to make new content available to the AI.
...
| Content harvest | Go / `bwctl harvest` | Fetches YouTube Shorts metadata and updates KB manifest |
...
Any advocacy organization can add itself to BWP by editing a single source file on GitHub and submitting a pull request. `CONTRIBUTING.md` in the repository provides step-by-step instructions requiring no local development environment. Organizations that adopt BWP's URL conventions for news and events paths will benefit from planned future aggregation features.
...
Non-technical contributors play an equally important role. Writing or improving knowledge base entries in `site/kb/`, contributing FAQ entries, submitting events, curating YouTube educational content, and verifying organization information are all valuable contributions that require no programming knowledge.
...
- **Repository:** https://github.com/hollingv/born-whole-project
- **Website:** https://bornwhole.org
- **Contributing:** https://github.com/hollingv/born-whole-project/blob/main/CONTRIBUTING.md
- **Organizations:** https://bornwhole.org/organizations.html
